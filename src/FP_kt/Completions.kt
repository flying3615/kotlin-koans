...
fun NavigableSet<NGram>.complete(input: String): List<String> {
    ...
    val seq = (canonicalisedInput.lastIndex + 1 downTo 0).map {
...
private fun String.pairwise() = mapIndexed { index, char -> if (index + 1 <= lastIndex) "$char${this[index + 1]}" else "" }.dropLast(1)
